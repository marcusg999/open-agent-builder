...
export const seedCinematicAdTemplate = mutation({
  ...
  handler: async (ctx) => {
    ...
    const existing = await ctx.db
      .query("workflows")
      .withIndex("by_customId", (q) => q.eq("customId", "cinematic-ad-agent-suite"))
      .first();
    
    if (existing) {
              ('Workflow already exists, skipping seed');
      return { success: false, message: "Workflow already exists" };
    }
    ...
    "trend": "description of how to leverage it"
...

        {
          ...
          data: {
            ...
            description: "Passes data through (web search handled by agents)",
            config: {}
          }
        },
